...
for _ in range(n):
    ...
    for i in range(n):
        if i not in processed and original_processes[i][1] <= current_time and original_processes[i][3] < min_pr:
...
for i in range(n):
    print(f"P{original_processes[i][0]}\t{original_processes[i][1]}\t{original_processes[i][2]}\t{original_processes[i][3]}\t{ct[i]}\t{wt[i]}\t{tat[i]}")
